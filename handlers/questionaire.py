...
async def fill_profile(message: Message, state: FSMContext):
    ...
    await message.answer("Давай начнем, введи свое имя", reply_markup=profile(message.from_user.first_name))
...
async def form_name(message: Message, state: FSMContext):
    ...
    await message.answer("Укажи свой возраст", reply_markup=rmk)
...
async def form_age(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        await message.answer("Укажи свой пол", reply_markup=profile(["Парень", "Девушка"]))
    else:
